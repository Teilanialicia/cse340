    ...
    return [
        ...
        .custom(async (inv_id) => {
          try{
            ...
            if (!data)
              throw new Error("Inventory does not exist with that id. Please try again.");
          }
          catch{
            throw new Error("There was an error checking the inventory before processing.");
          }
        }),
...
validate.checkUpdateInventoryData = async function(req, res, next) {
  const { inv_id, inv_make, inv_model, inv_description, inv_image, inv_thumbnail, inv_price, inv_year, inv_miles, inv_color } = req.body
  let errors = []
  errors = validationResult(req)
  if (!errors.isEmpty()) {
    let nav = await utilities.getNav()
    let classification_options = await invModel.getClassifications();
    res.render("./edit/edit-inventory", {
      errors,
      ...
      inv_miles, 
      inv_color,
      classification_options: classification_options.rows
    })
    return
  }
  next()
